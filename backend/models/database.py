...
def init_db():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            """)
            cur.execute("""
                INSERT IGNORE INTO defect_types VALUES
        ...
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO defect_reports (
                    document_no, received_date, customer_name, defect_type,
                    ai_defect_type, ai_confidence,
                ...
                (doc_no, received, customer_name, defect_type,
                 ai_defect_type, ai_confidence,
                 product_name, product_no, part_name, process_name, lot_no,
                 delivery_quantity, defect_quantity,
                 handler, author_name, delivery_date, issue_date),
...
def list_reports(status: str = None) -> list[dict]:
    with get_conn() as conn:
        with conn.cursor() as cur:
            if status:
                cur.execute(
                    "SELECT * FROM defect_reports WHERE report_status = %s ORDER BY created_at DESC",
                    (status,)
                )
            else:
                cur.execute("SELECT * FROM defect_reports ORDER BY created_at DESC")
            return cur.fetchall()


ALLOWED_UPDATE_FIELDS = {
    "defect_type", "defect_code", "product_name", "product_no",
    "part_name", "process_name", "lot_no", "customer_name",
    "delivery_quantity", "defect_quantity", "delivery_date", "issue_date",
...
def insert_image(report_id: int, image_path: str, image_type: str = None, image_description: str = None) -> int:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
